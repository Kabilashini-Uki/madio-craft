...
import { 
  ...
  FiBriefcase, FiShoppingBag, FiArrowLeft, FiAlertCircle, FiCheckCircle
} from 'react-icons/fi';
...
const Register = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    yearsOfExperience: '', craftDescription: '', location: '', specialties: []
  });
  ...
  const handleChange = (e) => {
    ...
    if (name === 'craftCategory') {
      setFormData(prev => ({ ...prev, craftCategory: value, specialties: value !== 'other' ? [value] : [] }));
    } else {
  ...
  const validateStep = () => {
    if (step === 1) {
      ...
      if (emailStatus === 'taken') { toast.error('This email is already registered'); return false; }
      if (emailStatus === 'checking') { toast.error('Please wait while we check your email'); return false; }
    }
    if (step === 2 && formData.role === 'artisan') {
      if (!formData.businessName?.trim()) { toast.error('Please enter your business name'); return false; }
      if (!formData.craftCategory) { toast.error('Please select your craft category'); return false; }
      if (!formData.location) { toast.error('Please select your location'); return false; }
    }
    return true;
  };

  const nextStep = () => { if (!validateStep()) return; setStep(step + 1); };
    ...
    try {
      ...
      if (dataToSend.role === 'artisan') {
        dataToSend.artisanProfile = {
          ...
          specialties: dataToSend.specialties.length > 0 ? dataToSend.specialties : (dataToSend.craftCategory ? [dataToSend.craftCategory] : []),
          yearsOfExperience: dataToSend.yearsOfExperience || 0,
        };
        delete dataToSend.businessName; delete dataToSend.craftCategory;
        delete dataToSend.yearsOfExperience; delete dataToSend.craftDescription; delete dataToSend.specialties;
      }
  ...
  const emailIndicator = () => {
    if (emailStatus === 'checking') return <div className="animate-spin rounded-full h-4 w-4 border-2 border-gray-400 border-t-transparent" />;
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-50 to-gray-100 py-12 px-4">
      <div className="max-w-md w-full">
        <button onClick={() => navigate('/')} className="flex items-center space-x-2 text-gray-600 hover:text-primary mb-6 transition-colors">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-gray-600">Start your journey with Sri Lanka's finest artisans</p>
          </div>
                ...
                <div className="space-y-2">
                  ...
                  {emailStatus === 'taken' && (
                    <motion.p initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }}
                      ...
                      <FiAlertCircle className="h-3.5 w-3.5" /> This email is already registered. <Link to="/login" className="underline font-medium">Sign in instead?</Link>
                    </motion.p>
                  )}
              ...
              <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <button type="button" onClick={() => { setFormData({...formData, role: 'buyer'}); nextStep(); }}
                    ...
                    <button type="button" onClick={() => setFormData({...formData, role: 'artisan'})}
                ...
                <div className="flex space-x-4">
                  <button type="button" onClick={prevStep} className="flex-1 py-3 px-4 border border-gray-300 rounded-xl text-gray-700 hover:bg-gray-50 transition-colors">Back</button>
                  <button type="button" onClick={nextStep}
